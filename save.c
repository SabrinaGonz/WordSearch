...
char listUpperCase(char* word, int letter);
char matrixUpperCase(char** arr, int letter, int i, int j, int type);
...
int searchHorizL(char** arr, int n, char* word){
    ...
    for(i = 0; i < n; i++){
        for(j = 0; j < n - size; j++){
            int letter = 0;
            while(letter < size){
                char x = listUpperCase(word, letter);
                char y = matrixUpperCase(arr, letter, i, j, 1);
...
int searchHorizR(char** arr, int n, char* word){
    ...
    for(i = 0; i < n; i++){
        for(j = n - 1; j >= size; j--){
            ...
            while(letter < size){
                    char x = listUpperCase(word, letter);
                    char y = matrixUpperCase(arr, letter, i, j, 2);
...
int searchVertDwn(char** arr, int n, char* word){
    ...
    for(i = 0; i < n; i++){
        for(j = 0; j < n-size; j++){
            ...
            while(letter < size){
                    char x = listUpperCase(word, letter);
                    char y = matrixUpperCase(arr, letter, i, j, 3);
                
                if(x == y){
                    if(letter == size - 1){
                        matrixlowerCase(arr, size, i, j, 3);
                        return 1;
                    }
                }
                else{
    ...
    for(i = 0; i < n; i++){
        for(j = 0; j < n; j++){
            ...
            while(letter < size
                ...
                char x = listUpperCase(word, letter);
                char y = matrixUpperCase(arr, letter, i, j, 4);
...
int searchDiagRL(char** arr, int n, char* word){
    ...
    for(i = 0; i < n; i++){
        for(j = n - 1; j > 0; j--){
            int letter = 0;
            whil e(letter < size && (i + letter < n) && (j + letter < n)){
                char x = listUpperCase(word, letter);
                char y = matrixUpperCase(arr, letter, i, j, 5);
...
char lowerCase(char** arr, int letter, int i, int j, int type){
    char temp;
    ...
    switch(type){
        case 1:
            ...
            break;
        case 3:
            if(*(*(arr+j+letter)+i) >= 'A' && *(*(arr+j+letter)+i) <= 'Z'){
                temp = *(*(arr+j+letter)+i);
            } else{
                temp = *(*(arr+j+letter)+i) - 32;
            }
            return temp;
            break;
        case 4:
            if(*(*(arr+i+letter)+j+letter) >= 'A' && *(*(arr+i+letter)+j+letter) <= 'Z'){
                temp = *(*(arr+i+letter)+j+letter);
            } else{
                temp = *(*(arr+i+letter)+j+letter) - 32;
            }
            return temp;
            break;
        case 5:
